...
bool CGame :: EventPlayerBotCommand( CGamePlayer *player, string command, string payload )
{
	...
	if( player->GetSpoofed( ) && ( AdminCheck || RootAdminCheck || IsOwner( User ) ) )
	{
		...
		if( !m_Locked || RootAdminCheck || IsOwner( User ) )
		{
			...
			******************/
			...
			else if ( Command == "normalcountdown" && !Payload.empty( ) )
			{
				if ( m_GHost->m_HideCommands )
					HideCommand = true;
	...
	*********************/
	...
	else if( ( Command == "stats" || Command == "s" ) && GetTime( ) - player->GetStatsSentTime( ) >= 5 )
	{
	...
	else if( ( Command == "statsdota" || Command == "sd" || Command == "ds" || Command == "dotastats" ) && GetTime( ) - player->GetStatsDotASentTime( ) >= 5 )
	{
		string StatsUser = User;
